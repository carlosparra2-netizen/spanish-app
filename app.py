...
def create_student_doc(drive, docs, folder_id, student_name, task_id, level, title, prompt_text, rubric, target, response_text, ai_feedback):
    try:
        ...
        feedback_section = ai_feedback if ai_feedback else "⚠️ AI feedback not available."
        full_text = (
...
def list_student_docs(drive, folder_id):
    try:
        ...
        res = drive.files().list(
            q=q, fields="files(id,name,createdTime,webViewLink)", orderBy="createdTime desc"
        ).execute()
...
# ── CSS + JS Timer (runs in browser, not server) ──────────
st.markdown("""
<style>
...
.tips-item {
    ...
    margin-bottom: 0.4rem; font-size: 0.9rem; color: #7f1d1d !important;
}
/* ── JS-powered timer display ── */
#js-timer {
    font-family: 'Fraunces', serif;
    font-size: 2.8rem; font-weight: 900;
    text-align: center; padding: 0.5rem;
    border-radius: 12px; margin-bottom: 0.5rem;
    color: #047857; background: rgba(16,185,129,0.08);
    border: 1px solid rgba(16,185,129,0.2);
}
#js-timer.yellow { color: #92400e !important; background: rgba(217,119,6,0.08); border-color: rgba(217,119,6,0.2); }
#js-timer.red    { color: #991b1b !important; background: rgba(220,38,38,0.08); border-color: rgba(220,38,38,0.3); }
/* Speaking timer */
#js-speak-timer {
    font-family: 'Fraunces', serif;
    font-size: 2.8rem; font-weight: 900;
...
#js-speak-timer.yellow { color: #92400e !important; background: rgba(217,119,6,0.08); border-color: rgba(217,119,6,0.2); }
#js-speak-timer.red    { color: #991b1b !important; background: rgba(220,38,38,0.08); border-color: rgba(220,38,38,0.3); }
...
.feedback-box {
    ...
    font-size: 0.95rem; line-height: 1.8; color: #1a1a2e !important; margin-top: 1rem;
}
.feedback-title { font-weight: 700; font-size: 1rem; color: #047857 !important; margin-bottom: 0.6rem; }
.flashcard-front {
    background: #f0f4ff; border: 2px solid #c7d2fe;
    border-radius: 20px; padding: 2.5rem; text-align: center; min-height: 160px;
    display: flex; flex-direction: column; align-items: center; justify-content: center;
}
.flashcard-word { font-family: 'Fraunces', serif; font-size: 2.5rem; font-weight: 700; color: #1b3a6b !important; }
.flashcard-back { background: #fefce8; border: 2px solid #fde68a; border-radius: 20px; padding: 2.5rem; text-align: center; }
.flashcard-back * { color: #1a1a2e !important; }
.stTextArea textarea {
    background: #ffffff !important; border: 2px solid #c7d2fe !important;
    color: #1a1a2e !important; border-radius: 12px !important;
    font-family: 'DM Sans', sans-serif !important; font-size: 1rem !important; line-height: 1.7 !important;
}
.stTextArea textarea:focus { border-color: #4f46e5 !important; box-shadow: 0 0 0 3px rgba(79,70,229,0.15) !important; }
.stTextArea textarea:disabled { background: #f1f5f9 !important; color: #94a3b8 !important; }
.stButton > button { border-radius: 10px !important; font-family: 'DM Sans', sans-serif !important; font-weight: 600 !important; font-size: 1.1rem !important; }
div[data-testid="column"] .stButton > button {
    font-size: 1.3rem !important; font-weight: 700 !important; padding: 0.4rem 0 !important;
    min-height: 3rem !important; background-color: #4f46e5 !important; color: white !important; border: none !important;
}
div[data-testid="column"] .stButton > button:hover { background-color: #3730a3 !important; }
</style>
...
// ── Writing timer (10 min, runs in browser) ──
var writingTimerInterval = null;
function startWritingTimer(endTimestamp) {
    ...
    writingTimerInterval = setInterval(function() {
        ...
        el.className = remaining > 120 ? '' : remaining > 30 ? 'yellow' : 'red';
    }, 1000);
}

// ── Speaking timer (60 sec, runs in browser) ──
var speakTimerInterval = null;
function startSpeakTimer() {
    var el = document.getElementById('js-speak-timer');
    if (!el) return;
    el.style.display = 'block';
    var endTime = Math.floor(Date.now() / 1000) + 60;
    if (speakTimerInterval) clearInterval(speakTimerInterval);
    speakTimerInterval = setInterval(function() {
        var remaining = endTime - Math.floor(Date.now() / 1000);
        if (remaining <= 0) {
        ...
        el.className = remaining > 30 ? '' : remaining > 10 ? 'yellow' : 'red';
    }, 1000);
}
</script>
...
WRITING_TASKS = {
    "👤 Personal": [
        ("P-01","Novice High","Introduce yourself to a pen pal",
         ...
         "60–90 words", 60, 90, "Include name, age, location • 1 hobby/interest • 4 clear facts • E-mail format"),
        ("P-02","Novice High","E-mail to a new teacher",
         ...
         "60–90 words", 60, 90, "4 things teacher should know • Personal and specific • Appropriate register"),
        ("P-03","Intermediate Low","Yearbook bio",
         ...
         "70–100 words", 70, 100, "4 facts as student & person • Descriptive language • Organized structure"),
        ("P-04","Intermediate Low","E-mail to an exchange student",
         ...
         "80–110 words", 80, 110, "4 home activities with details • Welcoming tone • Connect to your personality"),
        ("P-05","Novice High","Daily routine facts",
         ...
         "60–80 words", 60, 80, "4 routine facts • Present tense verbs • Specific times/details"),
        ("P-06","Intermediate Low","Sports app profile",
         ...
         "80–110 words", 80, 110, "Self-introduction • 4 sports/activities • Explain why each matters to you"),
        ("P-07","Novice High","E-mail about four friends",
         ...
         "70–100 words", 70, 100, "4 friends named • 1 interesting fact each • Variety of descriptions"),
        ("P-08","Intermediate Low","Describe four family members",
         ...
         "90–120 words", 90, 120, "4 family members • What each does • Why each is important to you"),
        ("P-09","Intermediate Low","E-mail to Maria about your friends",
         ...
         "100–130 words", 100, 130, "4 friends described • Something in common • Why you enjoy time with them"),
        ("P-10","Novice High","People who have influenced you",
         ...
         "60–80 words", 60, 80, "4 people named • 1 sentence each • Explain why they matter"),
    ],
    "❓ Questions": [
        ("Q-01","Novice High","Questions for a new student",
         ...
         "50–80 words", 50, 80, "4 open-ended questions • No yes/no questions • Variety of question words"),
        ("Q-02","Novice High","Interview the school principal",
         ...
         "50–80 words", 50, 80, "4 questions about school life • Formal register • Go beyond basic facts"),
        ("Q-03","Intermediate Low","Questions for Maria about moving",
         ...
         "60–90 words", 60, 90, "4 questions • Ask for opinions and feelings • No simple fact questions"),
        ("Q-04","Intermediate Low","Questions for a family member",
         ...
         "60–90 words", 60, 90, "4 questions inviting stories • Open-ended and reflective • Invite memories"),
        ("Q-05","Intermediate Low","Questions for a professional athlete",
         ...
         "60–90 words", 60, 90, "4 deep questions • Career, training, personal life • Beyond salary/schedule"),
        ("Q-06","Intermediate Mid","Teen life survey questions",
         ...
         "70–100 words", 70, 100, "4 survey questions • School, social life, and goals • Invite thoughtful responses"),
        ("Q-07","Novice High","Questions about a career",
         ...
         "50–80 words", 50, 80, "4 career questions • Go beyond pay and hours • Ask about challenges and meaning"),
        ("Q-08","Intermediate Low","Questions for Maria about a holiday",
         ...
         "60–90 words", 60, 90, "4 questions • Description, comparison, and reflection • Not just listing facts"),
    ],
    "📍 Places": [
        ("C-01","Novice High","Things to do in your town",
         ...
         "70–100 words", 70, 100, "4 places/activities • Explain why each is enjoyable • E-mail format"),
        ("C-02","Intermediate Low","Weekend visit plans",
         ...
         "90–120 words", 90, 120, "4 places described • What makes each special • Welcoming and personal tone"),
        ("C-03","Intermediate Low","Fitness activities in your community",
         ...
         "80–110 words", 80, 110, "4 fitness activities • Where/when each happens • Descriptive details"),
        ("C-04","Intermediate Mid","Recommend your town",
         ...
         "100–130 words", 100, 130, "4 specific reasons • Persuasive tone • Details and examples for each"),
        ("C-05","Intermediate Low","What your town needs",
         ...
         "80–110 words", 80, 110, "4 things your town needs • Explain what each is • Why it would help students"),
        ("C-06","Novice High","Healthy activities at school",
         ...
         "60–90 words", 60, 90, "4 activities described • Connection to health • Where/when available"),
    ],
    "🇺🇸 Life in America": [
        ("E-01","Intermediate Mid","Shopping",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • Explanation + description + narrative • Past tense in part 3"),
        ("E-02","Intermediate Mid","Holidays and celebrations",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • Cultural explanation • Detailed description • Personal narrative"),
        ("E-03","Intermediate Mid","School sports",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • General → specific → personal • Vivid details in part 3"),
        ("E-04","Intermediate Mid","Social media and technology",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • General → specific use → personal experience"),
        ("E-05","Intermediate Mid","Volunteering and community service",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • Definition + example + personal narrative • Lesson learned"),
        ("E-06","Intermediate Mid","Family traditions",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • Explanation → specific tradition → personal memory"),
        ("E-07","Intermediate Mid","School clubs",
         ...
         "150–200 words", 150, 200, "All 3 parts addressed • Explanation + description + full narrative"),
    ],
}
...
with st.sidebar:
    ...
    if st.session_state.teacher_mode:
        ...
        if not st.session_state.name_entered:
            ...
            st.markdown("**Go to:**")
            nav_items = [
                ("🏠","Home","home"),("🃏","Vocabulario","vocab"),("⚡","Gramática","grammar"),
                ("📖","Lectura","reading"),("💬","Conversación","speaking"),("✍️","Escritura AAPPL","writing"),
            ]
            for icon, label, key in nav_items:
                active = st.session_state.screen == key
                if st.button(f"{icon}  {label}", use_container_width=True, type="primary" if active else "secondary"):
...
if st.session_state.teacher_mode:
    ...
    if not drive:
        st.warning("⚠️ Google Drive not connected. Check Streamlit Secrets.")
    else:
        ...
        with c1:
            st.markdown(f"📁 **{FOLDER_NAME} / {teacher_label}**")
        with c2:
        ...
        if not docs_list:
            ...
            for i, doc in enumerate(filtered):
                name = doc.get("name",""); link = doc.get("webViewLink","")
                created = doc.get("createdTime","")[:10] if doc.get("createdTime") else ""
                c1, c2, c3 = st.columns([4,2,2])
                with c1: st.markdown(f"**{name}**")
                with c2: st.markdown(f"📅 {created}")
                with c3:
                    if link: st.markdown(f"[📄 Open Doc]({link})")
                ...
                with gc2:
                    st.text_input("Note:", placeholder="Optional feedback...", key=f"note_{i}", label_visibility="collapsed")
                st.divider()
    st.stop()
...
if st.session_state.screen == "home":
    st.markdown(f'<div class="main-title" style="font-size:3rem">🇪🇸 ¡Español!</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════
    ...
    with c1:
        if st.button("← Prev", use_container_width=True):
            st.session_state.vocab_idx = (idx - 1) % len(cards); st.session_state.vocab_flipped = False; st.rerun()
    with c2:
        if st.button("🔙 Hide" if st.session_state.vocab_flipped else "👁 Reveal", use_container_width=True):
    ...
    with c4:
        if st.button("🔀 Shuffle", use_container_width=True):
            shuffled = list(VOCAB_SETS[st.session_state.vocab_set]); random.shuffle(shuffled)
            st.session_state.vocab_shuffled = shuffled; st.session_state.vocab_idx = 0; st.session_state.vocab_flipped = False; st.rerun()
...
# ═══════════════════════════════════════════════════════════
    ...
    if not st.session_state.grammar_pool:
        pool = list(GRAMMAR_QUESTIONS); random.shuffle(pool)
        st.session_state.grammar_pool = pool; st.session_state.grammar_idx = 0
        st.session_state.grammar_score = 0; st.session_state.grammar_answered = 0
        st.session_state.grammar_selected = None
    pool = st.session_state.grammar_pool; idx = st.session_state.grammar_idx
    if idx >= len(pool):
        ...
        if st.button("🔄 Try Again", type="primary"): st.session_state.grammar_pool = []; st.rerun()
        st.stop()
    ...
    st.markdown(f"**{text.replace('___', '`___`')}**")
        ...
        for i, opt in enumerate(options):
            with cols[i % 2]:
                if st.button(opt, key=f"opt_{idx}_{i}", use_container_width=True):
                    st.session_state.grammar_selected = i; st.session_state.grammar_answered += 1
                    if i == answer: st.session_state.grammar_score += 1
                    st.rerun()
    else:
        cols = st.columns(2)
        for i, opt in enumerate(options):
            with cols[i % 2]:
                if i == answer: st.success(f"✓ {opt}")
                elif i == selected: st.error(f"✗ {opt}")
                else: st.button(opt, key=f"opt_d_{idx}_{i}", disabled=True, use_container_width=True)
        if selected == answer: st.success(f"✓ Correct! {explanation}")
        else: st.error(f"✗ Not quite. Correct: **{options[answer]}**. {explanation}")
        if st.button("Next question →", type="primary"):
            st.session_state.grammar_idx += 1; st.session_state.grammar_selected = None; st.rerun()
...
# ═══════════════════════════════════════════════════════════
# SPEAKING  — timer runs in browser via JavaScript
# ═══════════════════════════════════════════════════════════
    ...
    # JS timer — no server load at all
    st.markdown('<div id="js-speak-timer">1:00</div>', unsafe_allow_html=True)
    ...
    with c2:
        # Button triggers JS timer — no server rerun needed
        st.markdown("""
        <button onclick="startSpeakTimer()" style="
            width:100%; padding:0.6rem; border-radius:10px; border:1px solid #d1d5db;
            background:#f9fafb; font-family:'DM Sans',sans-serif; font-size:1.1rem;
            font-weight:600; cursor:pointer; color:#1a1a2e;">
            ⏱ Start 60s Timer
        </button>""", unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════
# WRITING  — timer runs in browser via JavaScript
# ═══════════════════════════════════════════════════════════
    ...
    if cat != st.session_state.writing_cat:
        st.session_state.writing_cat = cat; st.session_state.writing_idx = 0
        st.session_state.writing_started = False; st.session_state.writing_timer_end = None; st.rerun()
    ...
    if not st.session_state.writing_started and not already_done:
        ...
        if st.button("✍️  Start Writing — Begin Timer", type="primary", use_container_width=True):
            ...
            st.session_state.writing_timer_end = int(time.time()) + 600
            st.rerun()
    ...
    # ── Writing area ──
        ...
        with col2:
            ...
            is_locked = already_done

            # ── JS Timer — runs entirely in browser ──
            if not already_done and st.session_state.writing_timer_end:
                end_ts = st.session_state.writing_timer_end
                st.markdown(f'<div id="js-timer">10:00</div>', unsafe_allow_html=True)
                st.markdown(f"""
<script>
(function() {{
    var endTs = {end_ts};
    ...
    if (!el) return;
    function tick() {{
        var remaining = endTs - Math.floor(Date.now() / 1000);
        if (remaining <= 0) {{
            el.textContent = "⏰ Time's up!";
            el.className = 'red';
            return;
    ...
    tick();
            ...
            if not is_locked:
                ...
                for ci, char in enumerate(all_chars):
                    with char_cols[ci]:
                        if st.button(char, key=f"ch_{key_resp}_{ci}", use_container_width=True):
                            cur = st.session_state.get(f"textarea_{key_resp}", saved.get("text",""))
            ...
            response_text = st.text_area(
                ...
                value=init_val, height=320, disabled=is_locked,
            ...
            if already_done:
                ...
                if doc_url: st.markdown(f"📄 [Open your Google Doc]({doc_url})")
                ai_fb = saved.get("ai_feedback"); ai_err = saved.get("ai_error")
                if ai_fb:
                ...
                if st.button("📤 Submit & Get AI Feedback", type="primary", use_container_width=True,
                    ...
                    with st.spinner("🤖 Generating AI feedback..."):
                        ai_feedback, ai_error = get_ai_feedback(
                            st.session_state.student_name, tid, level, title, prompt_text, rubric, target, response_text)
                    ...
                    st.session_state.writing_responses[key_resp] = {
                        "text": response_text, "submitted": True, "words": words,
                        "task": f"{tid} — {title}", "level": level,
                        "doc_url": doc_url, "ai_feedback": ai_feedback, "ai_error": ai_error,
                    }
